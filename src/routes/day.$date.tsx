        ...
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <div className="flex items-center gap-1 flex-1 justify-center">
            <Link
              to="/day/$date"
              params={{ date: prev }}
              className="px-2 py-1 text-sm hover:bg-muted rounded-md"
            >
              ←
            </Link>
            ...
            {date !== today && (
              <Link
                to="/day/$date"
                params={{ date: today }}
                className="text-xs text-muted-foreground hover:text-foreground ml-1 px-2 py-1 hover:bg-muted rounded-md"
              >
                Today
              </Link>
            )}
    ...
    <div>
      <div className="flex items-baseline gap-2 mb-2">
        ...
        <span className="text-sm text-muted-foreground">
          {activity.duration_hrs}h ·{" "}
          {String(activity.window_start).padStart(2, "0")}:00–
          {String(activity.window_end).padStart(2, "0")}:00
        </span>
      </div>
...
function WindowCard({ window: w }: { window: ScoredWindow }) {
  ...
  return (
    <div
      className={`border rounded-lg overflow-hidden ${w.excluded ? "opacity-60" : ""}`}
    >
      <button
...
function ConstraintRow({
  ...
  return (
    <div className="grid grid-cols-[7rem_1fr_1fr_3rem] gap-x-4 items-start text-sm">
      ...
      <div
        className={`text-right tabular-nums ${result.passed ? "text-green-600" : "text-red-500"}`}
      >
        {result.passed ? `${pct}%` : "✗"}
      </div>
    </div>
...
function describeRequested(c: Constraint): string {
  switch (c.type) {
    ...
    case "wind_dir": {
      const dirs = (c.preferred ?? [])
        .map((d) => degreesToCompass(d))
        .join(", ");
      return `${dirs} ±${c.tolerance ?? 0}°`;
    }
    case "weather": {
      const list = (c.ideal ?? c.acceptable ?? []).map(
        (w) => weatherLabels[w as WeatherCode] ?? w,
      );
      return list.join(", ") || "—";
    }
    case "tide_height":
    ...
    case "temperature":
      return `${c.ideal_min ?? 0}–${c.ideal_max ?? 0}°C ideal`;
    default:
      return "—";
  }
}
...
function describeActual(c: Constraint, hours: HourlyData[]): string {
  ...
  switch (c.type) {
    ...
    case "weather": {
      ...
      hours.forEach((h) => {
        counts[h.weather_code] = (counts[h.weather_code] || 0) + 1;
      });
    ...
    case "temperature": {
      const avg = hours.reduce((s, h) => s + h.temperature_celsius, 0) / hours.length;
              ...
              >
                ...
                <td className="px-3 py-2 tabular-nums">
                  ...
                  <span className="text-muted-foreground">
                    F{msToBeaufort(h.wind_speed_ms)}
                  </span>
                </td>
                <td className="px-3 py-2">
                  {degreesToCompass(h.wind_dir_deg)}
                </td>
